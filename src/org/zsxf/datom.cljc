...
(defn ds-datom->datom2->zset-item [datom]
  (#?(:clj  zs2/zset-item
      :cljs zs/zset-item)
    (datom2 datom) (util/bool->weight (nth datom 4))))
...
(defn ds-tx-datoms->datoms2->zsets
  ...
  (into
    ...
    (comp
      (map (fn [d]
             #?(:clj  (zs2/hash-zset (ds-datom->datom2->zset-item d))
                :cljs (zs/zset [(ds-datom->datom2->zset-item d)])))))
    datoms))
